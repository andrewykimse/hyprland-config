...
ShellRoot {
    ...
    Variants {
        ...
        PanelWindow {
            ...
            /**
             * The pill only actually retracts while fullscreen AND nothing has
             * pulled it back into view. Peeking (superscript+grave) or pinning
             * sets `pill.held`, which feeds `modal` — so a fullscreen window no
             * longer permanently traps the pill off-screen and unclickable;
             * peek still works exactly like it does outside fullscreen.
             */
            readonly property bool retracted: monFullscreen && !modal
            ...
            mask: retracted ? hiddenRegion : (modal ? fullRegion : pillRegion)
            ...
            FocusScope {
                ...
                Pill {
                    ...
                    transform: Translate {
                        y: overlay.retracted ? -(pill.height + overlay.topGap) : 0
                        Behavior on y {
                            NumberAnimation {
                                duration: Motion.morph
                                easing.type: Motion.easeMorph
